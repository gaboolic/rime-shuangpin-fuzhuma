...
dictLines.forEach((line) => {
    // 假设每行的格式为 "字\t编码"
    if (line.indexOf("\t") != -1) {
        const [word, encoding] = line.split('\t');
        if (encoding.indexOf("'") == -1) {

            dictData[word] = encoding;
        }
    }
});

console.log("并不比 " + dictData['并不比'])

// 需要修改的YAML文件的路径
const yamlFilePath = path.join(__dirname, 'xiaohe4zi.dict.yaml');

// 需要修改的YAML文件的路径
const writeFilePath = path.join(__dirname, 'xiaohe4zi_deal.dict.yaml');
...
function updateMissingEncodings(filePath, writeFilePath) {
    ...
    lines.forEach((line) => {
        ...
        double_list = "e" + double_list.substring(0, 3) + double_list.charAt(double_list.length - 1)
